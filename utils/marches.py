...
class Marche:
    def __init__(
        self, name, time, left, right, time_moments, left_moments, right_moments
    ):
        self.name = name
        self.time = time
    ...
    def printMarche(self):
        print(
            ...
            % (
                ...
                self.toesoff_left,
                self.toesoff_right,
                self.heelstrike_left,
                self.heelstrike_right,
            )
        )
    ...
    def cycle(self, side):
        if side == "right":
            ...
            duration = round(self.heelstrike_right[1], 3) - round(
                self.heelstrike_right[0], 3
            )
        else:
            for i in range(len(self.time)):
            ...
            duration = round(self.heelstrike_left[1], 3) - round(
                self.heelstrike_left[0], 3
            )
