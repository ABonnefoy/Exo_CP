...
##
## Use: visualise the data acquired from walk experiments
##
##
##
...
# Init saved data
waist_se3_list = np.zeros((7, len(marche.time)))
right_foot_se3_list = np.zeros((7, len(marche.time)))
left_foot_se3_list = np.zeros((7, len(marche.time)))

# Create contact visuals
gv.addSphere('world/right_contact',.1, [1, 0, 0, 1])
gv.addSphere('world/left_contact',.1, [0, 1, 0, 1])
...
while k < len(marche.time):
    ...
    if t < t_hs_right[i_hs_right] <= t + dt:
        gv.setVisibility('world/right_contact', 'ON')
        i_hs_right+=1

    if t < t_to_right[i_to_right] <= t + dt:
        gv.setVisibility('world/right_contact', 'OFF')
        i_to_right+=1

    if t < t_hs_left[i_hs_left] <= t + dt:
        gv.setVisibility('world/left_contact', 'ON')
        i_hs_left+=1
        
    if t < t_to_left[i_to_left] <= t + dt:
        gv.setVisibility('world/left_contact', 'OFF')
        i_to_left+=1

    # Allocation saved data
    waist_se3_list[:,k] = pin.SE3ToXYZQUAT(robot.data.oMf[robot.model.getFrameId("Pelvis")])
    right_foot_se3_list[:,k] = pin.SE3ToXYZQUAT(robot.data.oMf[robot.model.getFrameId("RightFoot")])
    left_foot_se3_list[:,k] = pin.SE3ToXYZQUAT(robot.data.oMf[robot.model.getFrameId("LeftFoot")])  

    gv.applyConfiguration('world/right_contact',right_foot_se3_list[:3,k].tolist()+[1, 0, 0, 0])
    gv.applyConfiguration('world/left_contact',left_foot_se3_list[:3,k].tolist()+[1, 0, 0, 0])

    k += 1
    # k = len(marche.time)
    t+=dt

save_visu_data(path, waist_se3_list, right_foot_se3_list, left_foot_se3_list)
